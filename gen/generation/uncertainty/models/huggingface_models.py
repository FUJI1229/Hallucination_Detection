...
class HuggingfaceModel(BaseModel):
    ...
    def __init__(self, model_name, stop_sequences=None, max_new_tokens=None):
        ...
        if 'llama' in model_name.lower():
            llama_model_kwargs = {}
            fourbit = False
            if model_name.endswith('-4bit') or '70b' in model_name.lower():
    ...
    def encode_hidden(self, input_text, return_layers):
        """
        Run without generation and return only the final-token hidden states
        ...
        # return_layers format: "min,interval"
        min_layer = int(return_layers.split(",")[0])
        interval = int(return_layers.split(",")[1])

        n_layers = len(hidden_states)
        extracted_layers = list(range(min_layer, n_layers, interval))

        last_token_idx = inputs["input_ids"].shape[1] - 1

        # [n_layers_selected, hidden_dim]
        hidden = torch.stack([
            hidden_states[layer][0, last_token_idx]
